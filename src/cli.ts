...
function applyFix(targetPath: string, result: AnalysisResult): void {
  ...
  // Docker treats '#' as a comment only at column 1. An inline comment would be
  // read as part of the pattern, so the rule would never match a real build.
  // Put each reason on its own line above the bare pattern.
  const block = result.suggestedRules.map((r) => `# ${r.reason}\n${r.pattern}`).join('\n');
  ...
  if (fs.existsSync(dockerignorePath)) {
    ...
    fs.writeFileSync(
      dockerignorePath,
      `# .dockerignore - generated by docker-context-scout\n${block}\n`,
      'utf-8'
    );
